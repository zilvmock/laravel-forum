<x-app-layout>
  <div class="md:m-4 md:p-2 mt-2 md:w-9/12">
    <x-layout.card class="rounded-none rounded-t md:rounded">
      <x-layout.top-bar>
        <x-slot:title>
          <h1 class="text-lg font-bold m-0">Create Category</h1>
        </x-slot:title>
      </x-layout.top-bar>
      <form action="{{route('store_category', $groupId)}}" method="post">
        @method('PUT')
        @csrf
        {{-- Category Title --}}
              ...
              <table class="table-fixed overflow-y-scroll">
                ...
                @foreach($articleData as $article)
                  <tr class="border-b border-secondary/10">
                    <th>
                      <label>
                        <input id="{{$article['id']}}" type="checkbox" class="checkbox checkbox-xs p-2 ml-2"/>
                      </label>
                    </th>
        ...
        <x-input.primary-button class="m-2" onclick="getChecked()">Create Category</x-input.primary-button>
