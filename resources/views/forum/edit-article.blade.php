  ...
  <x-layout.card class="m-4 p-4 w-full sm:w-fit md:w-fit lg:w-9/12 xl:w-10/12">
    <x-layout.card class="flex justify-between overflow-hidden bg-secondary/10 mb-2">
      <div class="flex">
        ...
        <h1 class="text-lg font-bold">Edit Article</h1>
      </div>
    </x-layout.card>
    <form action="{{route('update-article', $article->id)}}" method="post" enctype="multipart/form-data">
      ...
      <div class="bg-secondary/10 rounded p-4 mt-4 text-center">
        ...
        <x-input.text-input class="w-full text-center" maxlength="100" name="title" value="{{old('title', $article->title)}}"/>
      ...
      <div class="bg-secondary/10 rounded p-4 mt-4 text-center">
        ...
        <x-input.text-input class="w-full text-center" maxlength="100" name="tags" value="{{old('tags', $article->tags)}}"/>
      ...
      <div class="bg-secondary/10 rounded p-4 mt-4 text-center">
        ...
        <input id="selected-category" name="categoryId" type="hidden" value="{{$article->category_id}}"/>
        ...
        <script>
          let oldVal = '{{old('categoryId', $article->category_id)}}';
          if (oldVal !== '') {
            document.getElementById({{$article->category_id}}).selected=true;
          }
        </script>
        @error('categoryId')
        <x-informative.error-message class="py-2 mt-2">{{$message}}</x-informative.error-message>
