// noinspection JSJQueryEfficiency,EqualityComparisonWithCoercionJS,JSUnusedLocalSymbols,JSFunctionExpressionToArrowFunction,JSDuplicatedDeclaration,JSStringConcatenationToES6Template
    ...
    if (hasItems) {
      ...
      items.forEach(function (item, index) {
        ...
        if (item.thumb_url) {
          var image = $("<div>").css(
            "background-image",
            'url("' + item.thumb_url + "?timestamp=" + item.time + '")'
          );
        } else {
